...
function stochastic_product_production()
    ...
    # 训练模型；iteration_limit 可根据需要进行调整以改进下界
...
function compute_average_cost_and_carbon(simulations, products, resources, T, 
    ...
    avg_carbon = mean(total_carbons)
    return avg_cost, avg_carbon
end

# 计算单个模拟轨迹（选择第42个模拟轨迹）的总成本与总碳排放
...
# 计算所有模拟轨迹的总成本和总碳排放
all_costs, all_carbons = compute_all_costs_and_carbons(simulations, products, resources, T, 
    product_resource_params, inventory_costs, inventory_carbon, waste_costs, backorder_costs)

    using Printf

    # 计算最大值用于轴范围
    xmax = maximum(all_carbons)
    ymax = maximum(all_costs)
    
    # 绘图，坐标从 0 开始，不使用科学记数法
    p = scatter(all_carbons, all_costs;
        ...
        xlims = (0, xmax * 1.05),   # 从 0 到稍大于最大值，预留空间
        ylims = (0, ymax * 1.05),
        legend = false,
        grid = true
    )
    ...
    # 假设 cap 值统一（如 caps = [7000, 7000, ..., 7000]）
    cap_value = caps[1]  # 获取 cap 数值用于命名
    
    # 保存散点图为文件，文件名根据 cap 自动设置
    image_filename = joinpath(@__DIR__, @sprintf("model3_simulations_cap_%d.png", cap_value))
    savefig(p, image_filename)
    
    # 创建 DataFrame 并保存为 CSV，包含两个列：CarbonEmissions 和 TotalCost
    df = DataFrame( Cap = fill(cap_value, length(all_costs)), CarbonEmissions = all_carbons, TotalCost = all_costs)
    csv_filename = joinpath(@__DIR__, @sprintf("model3_simulation_results_cap_%d.csv", cap_value))
    CSV.write(csv_filename, df)
    
    println("Saved plot to: ", image_filename)
    println("Saved data to: ", csv_filename)
